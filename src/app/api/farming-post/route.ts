...
import { generateLinkedInFarmingPost, generateLinkedInFarmingPostWithImages } from '@/lib/linkedinFarming';
...
export async function POST(request: Request) {
  try {
    ...
    console.log('Generating farming post with images:', includeImages);
    
    let postBody;
    
    if (includeImages) {
      const postWithImages = await generateLinkedInFarmingPostWithImages('unsplash', 1);
